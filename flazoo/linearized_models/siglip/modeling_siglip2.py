...
# General docstring
_CONFIG_FOR_DOC = "FLASiglip2Config"


@dataclass
class FLASiglip2VisionOutput(ModelOutput):
    """
...
@dataclass
class FLASiglip2TextOutput(ModelOutput):
    """
...
@dataclass
class FLASiglip2Output(ModelOutput):
    """
    Args:
...
class FLASiglip2VisionEmbeddings(nn.Module):
    def __init__(self, config: FLASiglip2VisionConfig):
        super().__init__()
        self.config = config
...
class FLASiglip2Attention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""

    def __init__(self, config: Union[FLASiglip2VisionConfig, FLASiglip2TextConfig]):
        super().__init__()
        self.config = config
        self.embed_dim = config.hidden_size
...
class FLASiglip2EncoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: Union[FLASiglip2VisionConfig, FLASiglip2TextConfig], layer_idx : int = None):
        super().__init__()
        self.embed_dim = config.hidden_size
...
class FLASiglip2Encoder(nn.Module):
    """
...
class FLASiglip2VisionTransformer(nn.Module):
    def __init__(self, config: FLASiglip2VisionConfig):
        super().__init__()
        self.config = config
        embed_dim = config.hidden_size

        self.embeddings = FLASiglip2VisionEmbeddings(config)
        self.encoder = FLASiglip2Encoder(config)
        self.post_layernorm = nn.LayerNorm(embed_dim, eps=config.layer_norm_eps)
        self.use_head = True if not hasattr(config, "vision_use_head") else config.vision_use_head
        if self.use_head:
            self.head = FLASiglip2MultiheadAttentionPoolingHead(config)
        self._use_flash_attention_2 = config._attn_implementation == "flash_attention_2"

    @can_return_tuple
    @add_start_docstrings_to_model_forward(SIGLIP2_VISION_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=BaseModelOutputWithPooling, config_class=FLASiglip2VisionConfig)
    def forward(
        self,
        pixel_values: torch.FloatTensor,
        attention_mask: torch.Tensor,
...
class FLASiglip2TextEmbeddings(nn.Module):
    def __init__(self, config: FLASiglip2TextConfig):
        super().__init__()
        embed_dim = config.hidden_size
...
class FLASiglip2TextTransformer(nn.Module):
    def __init__(self, config: FLASiglip2TextConfig):
        super().__init__()
        self.config = config
        embed_dim = config.hidden_size
        self.embeddings = FLASiglip2TextEmbeddings(config)
        self.encoder = FLASiglip2Encoder(config)
    ...
    @replace_return_docstrings(output_type=BaseModelOutputWithPooling, config_class=FLASiglip2TextConfig)
    def forward(
        self,
...
class FLASiglip2PreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = FLASiglip2Config
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, FLASiglip2VisionEmbeddings):
            width = (
                self.config.vision_config.hidden_size
                if isinstance(self.config, FLASiglip2Config)
                else self.config.hidden_size
            )
            nn.init.normal_(module.position_embedding.weight, std=1 / np.sqrt(width))
        elif isinstance(module, nn.Embedding):
            default_flax_embed_init(module.weight)
        elif isinstance(module, FLASiglip2Attention):
        ...
        elif isinstance(module, FLASiglip2MLP):
            nn.init.xavier_uniform_(module.fc1.weight)
            nn.init.xavier_uniform_(module.fc2.weight)
            nn.init.normal_(module.fc1.bias, std=1e-6)
            nn.init.normal_(module.fc2.bias, std=1e-6)
        elif isinstance(module, FLASiglip2MultiheadAttentionPoolingHead):
            nn.init.xavier_uniform_(module.probe.data)
            nn.init.xavier_uniform_(module.attention.in_proj_weight.data)
            nn.init.zeros_(module.attention.in_proj_bias.data)
        elif isinstance(module, FLASiglip2Model):
            logit_scale_init = torch.log(torch.tensor(1.0))
            module.logit_scale.data.fill_(logit_scale_init)
            module.logit_bias.data.zero_()
        elif isinstance(module, FLASiglip2ForImageClassification):
            nn.init.normal_(
...
class FLASiglip2TextModel(FLASiglip2PreTrainedModel):
    config_class = FLASiglip2TextConfig

    def __init__(self, config: FLASiglip2TextConfig):
        super().__init__(config)
        self.text_model = FLASiglip2TextTransformer(config)
...
class FLASiglip2MultiheadAttentionPoolingHead(nn.Module):
...
class FLASiglip2VisionModel(FLASiglip2PreTrainedModel):
    config_class = FLASiglip2VisionConfig
    main_input_name = "pixel_values"

    def __init__(self, config: FLASiglip2VisionConfig):
        super().__init__(config)

        self.vision_model = FLASiglip2VisionTransformer(config)
...
class FLASiglip2Model(FLASiglip2PreTrainedModel):
    config_class = FLASiglip2Config

    def __init__(self, config: FLASiglip2Config):
        super().__init__(config)

        if not isinstance(config.text_config, FLASiglip2TextConfig):
            raise TypeError(
                "config.text_config is expected to be of type Siglip2TextConfig but is of type"
                f" {type(config.text_config)}."
            )

        if not isinstance(config.vision_config, FLASiglip2VisionConfig):
            raise TypeError(
        ...
        text_model = FLASiglip2TextModel._from_config(text_config)
        vision_model = FLASiglip2VisionModel._from_config(vision_config)
    ...
    @replace_return_docstrings(output_type=FLASiglip2Output, config_class=FLASiglip2Config)
    def forward(
        self,
        input_ids: Optional[torch.LongTensor] = None,
        pixel_values: Optional[torch.FloatTensor] = None,
...
class FLASiglip2ForImageClassification(FLASiglip2PreTrainedModel):
    main_input_name = "pixel_values"

    def __init__(self, config: FLASiglip2Config) -> None:
        super().__init__(config)
        ...
        vision_model = FLASiglip2VisionModel._from_config(config.vision_config)
        self.vision_model = vision_model.vision_model
...
__all__ = [
    "FLASiglip2Model",
    "FLASiglip2PreTrainedModel",
    "FLASiglip2TextModel",
    "FLASiglip2VisionModel",
    "FLASiglip2ForImageClassification",
]

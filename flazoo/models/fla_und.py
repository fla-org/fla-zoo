...
from flazoo.models.und.utils import ImageEmbeddings, Pooler
from transformers.utils.constants import IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD
from flazoo.models.und.utils import (
    VideoEmbeddings,
    VideoDecoderOutput,
...
class FLAVisionBlock(nn.Module):
    def __init__(self, config, layer_idx: int):
        ...
        if config.attn is not None and layer_idx in config.attn["layers"]:
            ...
            self.attn = get_fla_attn(config, layer_idx)
        ...
        if config.use_swiglu:
            ...
            self.channel_mixer = FLAVisionMLP(config)
...
class FLAVisionPreTrainedModel(PreTrainedModel):
    config_class = FLAVisionConfig
    base_model_prefix = "fla_vision"
...
class FLAVisionEncoder(nn.Module):
    def __init__(self, config) -> None:
        super().__init__()
        self.config = config
        self.blocks = nn.ModuleList(
            [
                FLAVisionBlock(config, layer_idx)
...
class FLAVisionModel(FLAVisionPreTrainedModel):
...
class FLAForImageClassification(FLAVisionPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_classes
        self.backbone = FLAVisionModel(
...
class FLAForMaskedImageModeling(FLAVisionPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.backbone = FLAVisionModel(
...
class FLAVideoBlock(nn.Module):
    def __init__(self, config, layer_idx: int):
        ...
        if config.use_swiglu:
            ...
            self.channel_mixer = FLAVideoMLP(config)
...
class FLAVideoEncoder(nn.Module):
    def __init__(self, config) -> None:
        super().__init__()
        self.config = config
        self.blocks = nn.ModuleList(
            [
                FLAVideoBlock(config, layer_idx)
                for layer_idx in range(config.num_hidden_layers)
            ]
        )
        self.gradient_checkpointing = False
...
class FLAVideoPreTrainedModel(PreTrainedModel):
    config_class = FLAVideoConfig
    supports_gradient_checkpointing = True
...
class FLAVideoModel(FLAVideoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
...
class FLAVideoDecoder(nn.Module):
    def __init__(self, config, num_patches):
        ...
        self.decoder_blocks = nn.ModuleList(
            [
                FLAVideoBlock(decoder_config, layer_idx)
...
class FLAForVideoPreTraining(FLAVideoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
        self.backbone = FLAVideoModel(config)
...
class FLAForVideoClassification(FLAVideoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

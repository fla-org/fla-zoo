...
class ABCVideoEncoder(nn.Module):
    def __init__(self, config) -> None:
        super().__init__()
        self.config = config
        self.blocks = nn.ModuleList(
            [ABCVideoBlock(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.gradient_checkpointing = False
...
class ABCVideoPreTrainedModel(PreTrainedModel):
    config_class = ABCVideoConfig
...
class ABCVideoModel(ABCVideoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
...
class ABCVideoDecoder(nn.Module):
...
class ABCForVideoPreTraining(ABCVideoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
        ...
        self.decoder = ABCVideoDecoder(config, num_patches=self.backbone.embeddings.num_patches)
...
class ABCForVideoClassification(ABCVideoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

...
class RetNetVisionBlock(nn.Module):
    def __init__(self, config, layer_idx: int):
        ...
        self.channel_mixer = RetNetVisionChannelMixer(config)
...
class RetNetVideoBlock(nn.Module):
    def __init__(self, config, layer_idx: int):
        ...
        self.channel_mixer = RetNetVideoChannelMixer(config)
        if config.attn is not None and layer_idx in config.attn['layers']:
            self.train_scan_type = 'uni-scan'
            self.test_scan_type = 'uni-scan'
        else:
...
class RetNetVideoEncoder(nn.Module):
    def __init__(self, config) -> None:
        super().__init__()
        self.config = config
        self.blocks = nn.ModuleList(
            [RetNetVideoBlock(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.gradient_checkpointing = False
...
class RetNetVideoPreTrainedModel(PreTrainedModel):
    config_class = RetNetVideoConfig
...
class RetNetVideoModel(RetNetVideoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config

        self.embeddings = VideoEmbeddings(config)
        self.encoder = RetNetVideoEncoder(config)
...
class RetNetVideoDecoder(nn.Module):
    def __init__(self, config, num_patches):
        ...
        self.decoder_blocks = nn.ModuleList(
            [RetNetVideoBlock(decoder_config, layer_idx) for layer_idx in range(decoder_config.num_hidden_layers)]
        )
...
class RetNetForVideoPreTraining(RetNetVideoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
        self.backbone = RetNetVideoModel(config)
        ...
        self.decoder = RetNetVideoDecoder(config, num_patches=self.backbone.embeddings.num_patches)
...
class RetNetForVideoClassification(RetNetVideoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

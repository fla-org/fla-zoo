...
from fla.modules import GatedMLP as MesaNetSwiGLU
...
class MesaNetVisionMLP(nn.Module):
...
class MesaNetVisionBlock(nn.Module):
    def __init__(self, config, layer_idx: int):
        ...
        if config.use_swiglu:
            ...
            self.channel_mixer = MesaNetSwiGLU(
                hidden_size=config.hidden_size,
            )
        else:
            self.channel_mixer = MesaNetVisionMLP(config)
...
class MesaNetVisionPreTrainedModel(PreTrainedModel):
    config_class = MesaNetVisionConfig
    base_model_prefix = "mesa_net_vision"
...
class MesaNetVisionEncoder(nn.Module):
    def __init__(self, config) -> None:
        super().__init__()
        self.config = config
        self.blocks = nn.ModuleList(
            [
                MesaNetVisionBlock(config, layer_idx)
...
class MesaNetVisionModel(MesaNetVisionPreTrainedModel):
    def __init__(self, config, add_pooling_layer=True, use_mask_token=False):
        super().__init__(config)
        self.config = config
        self.embeddings = ImageEmbeddings(config, use_mask_token=use_mask_token)
        self.encoder = MesaNetVisionEncoder(config)
...
class MesaNetForImageClassification(MesaNetVisionPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_classes
        self.backbone = MesaNetVisionModel(
...
class MesaNetForMaskedImageModeling(MesaNetVisionPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.backbone = MesaNetVisionModel(
...
class MesaNetVideoMLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(config.hidden_size, config.channel_mixer_dim),
...
class MesaNetVideoBlock(nn.Module):
    def __init__(self, config, layer_idx: int):
        super().__init__()
        ...
        if config.use_swiglu:
            ...
            self.channel_mixer = MesaNetVideoMLP(config)
...
class MesaNetVideoEncoder(nn.Module):
    def __init__(self, config) -> None:
        super().__init__()
        self.config = config
        self.blocks = nn.ModuleList(
            [
                MesaNetVideoBlock(config, layer_idx)
                for layer_idx in range(config.num_hidden_layers)
            ]
        )
        self.gradient_checkpointing = False
...
class MesaNetVideoPreTrainedModel(PreTrainedModel):
    config_class = MesaNetVideoConfig
    supports_gradient_checkpointing = True
...
class MesaNetVideoModel(MesaNetVideoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config

        self.embeddings = VideoEmbeddings(config)
        self.encoder = MesaNetVideoEncoder(config)
        self.layernorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
...
class MesaNetVideoDecoder(nn.Module):
    def __init__(self, config, num_patches):
        ...
        self.decoder_blocks = nn.ModuleList(
            [
                MesaNetVideoBlock(decoder_config, layer_idx)
...
class MesaNetForVideoPreTraining(MesaNetVideoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
        self.backbone = MesaNetVideoModel(config)
        ...
        self.decoder = MesaNetVideoDecoder(
...
class MesaNetForVideoClassification(MesaNetVideoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

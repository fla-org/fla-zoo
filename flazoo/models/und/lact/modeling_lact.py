...
class LaCTVisionBlock(nn.Module):
    def __init__(self, config, layer_idx: int):
        ...
        if config.use_swiglu:
            ...
            self.channel_mixer = LaCTSwiGLU(
                hidden_size=config.hidden_size,
            )
        else:
            self.channel_mixer = LaCTVisionMLP(config)
    ...
    def forward(
        ...
        **kwargs: Unpack[Dict],
        ...
        outputs = (hidden_states, _, past_key_values)

        return outputs


class LaCTVisionPreTrainedModel(PreTrainedModel):
    config_class = LaCTVisionConfig
    base_model_prefix = "lact_vision"
...
class LaCTVisionEncoder(nn.Module):
    def __init__(self, config) -> None:
        super().__init__()
        self.config = config
        self.blocks = nn.ModuleList(
            [
                LaCTVisionBlock(config, layer_idx)
...
class LaCTForImageClassification(LaCTVisionPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_classes
        self.backbone = LaCTVisionModel(
...
class LaCTForMaskedImageModeling(LaCTVisionPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.backbone = LaCTVisionModel(
...
class LaCTVideoBlock(nn.Module):
    def __init__(self, config, layer_idx: int):
        super().__init__()
        ...
        if config.use_swiglu:
            ...
            self.channel_mixer = LaCTVideoMLP(config)
...
class LaCTVideoEncoder(nn.Module):
    def __init__(self, config) -> None:
        super().__init__()
        self.config = config
        self.blocks = nn.ModuleList(
            [
                LaCTVideoBlock(config, layer_idx)
                for layer_idx in range(config.num_hidden_layers)
            ]
        )
        self.gradient_checkpointing = False
...
class LaCTVideoPreTrainedModel(PreTrainedModel):
    config_class = LaCTVideoConfig
    base_model_prefix = "lact_video"
    main_input_name = "pixel_values"
    _no_split_modules = ["VideoEmbeddings", "LaCTVideoBlock"]
    supports_gradient_checkpointing = True
...
class LaCTVideoModel(LaCTVideoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config

        self.embeddings = VideoEmbeddings(config)
        self.encoder = LaCTVideoEncoder(config)
        self.layernorm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
...
class LaCTVideoDecoder(nn.Module):
    def __init__(self, config, num_patches):
        ...
        self.decoder_blocks = nn.ModuleList(
            [
                LaCTVideoBlock(decoder_config, layer_idx)
...
class LaCTForVideoPreTraining(LaCTVideoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
        self.backbone = LaCTVideoModel(config)
...
class LaCTForVideoClassification(LaCTVideoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

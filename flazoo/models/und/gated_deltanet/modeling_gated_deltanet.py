...
class GatedDeltaNetVideoBlock(nn.Module):
    def __init__(self, config, layer_idx: int):
        super().__init__()
...
class GatedDeltaNetVideoEncoder(nn.Module):
    def __init__(self, config) -> None:
        super().__init__()
        self.config = config
        self.blocks = nn.ModuleList(
            [GatedDeltaNetVideoBlock(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.gradient_checkpointing = False
...
class GatedDeltaNetVideoPreTrainedModel(PreTrainedModel):
    config_class = GatedDeltaNetVideoConfig
...
class GatedDeltaNetVideoModel(GatedDeltaNetVideoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config

        self.embeddings = VideoEmbeddings(config)
        self.encoder = GatedDeltaNetVideoEncoder(config)
...
class GatedDeltaNetVideoDecoder(nn.Module):
...
class GatedDeltaNetForVideoPreTraining(GatedDeltaNetVideoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
        self.backbone = GatedDeltaNetVideoModel(config)
        ...
        self.decoder = GatedDeltaNetVideoDecoder(config, num_patches=self.backbone.embeddings.num_patches)
...
class GatedDeltaNetForVideoClassification(GatedDeltaNetVideoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

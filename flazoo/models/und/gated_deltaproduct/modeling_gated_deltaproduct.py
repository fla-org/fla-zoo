...
class GatedDeltaProductVisionMLP(nn.Module):
...
class GatedDeltaProductVisionBlock(nn.Module):
    def __init__(self, config, layer_idx: int):
        ...
        self.channel_mixer = GatedDeltaProductVisionMLP(config)
...
class GatedDeltaProductVisionPreTrainedModel(PreTrainedModel):
    config_class = GatedDeltaProductVisionConfig
...
class GatedDeltaProductVisionEncoder(nn.Module):
    def __init__(self, config) -> None:
        super().__init__()
        self.config = config
        self.blocks = nn.ModuleList(
            [
                GatedDeltaProductVisionBlock(config, layer_idx)
...
class GatedDeltaProductVisionModel(GatedDeltaProductVisionPreTrainedModel):
    def __init__(self, config, add_pooling_layer=True, use_mask_token=False):
        super().__init__(config)
        self.config = config
        self.embeddings = ImageEmbeddings(config, use_mask_token=use_mask_token)
        self.encoder = GatedDeltaProductVisionEncoder(config)
...
class GatedDeltaProductForImageClassification(GatedDeltaProductVisionPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_classes
        self.backbone = GatedDeltaProductVisionModel(
...
class GatedDeltaProductForMaskedImageModeling(GatedDeltaProductVisionPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.backbone = GatedDeltaProductVisionModel(
...
class GatedDeltaProductVideoMLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(config.hidden_size, config.channel_mixer_dim),
...
class GatedDeltaProductVideoBlock(nn.Module):
    def __init__(self, config, layer_idx: int):
        super().__init__()
        ...
        self.channel_mixer = GatedDeltaProductVideoMLP(config)
        if config.attn is not None and layer_idx in config.attn["layers"]:
            self.train_scan_type = "uni-scan"
            self.test_scan_type = "uni-scan"
        else:
...
class GatedDeltaProductVideoEncoder(nn.Module):
    def __init__(self, config) -> None:
        super().__init__()
        self.config = config
        self.blocks = nn.ModuleList(
            [
                GatedDeltaProductVideoBlock(config, layer_idx)
                for layer_idx in range(config.num_hidden_layers)
            ]
        )
        self.gradient_checkpointing = False
...
class GatedDeltaProductVideoPreTrainedModel(PreTrainedModel):
    config_class = GatedDeltaProductVideoConfig
...
class GatedDeltaProductVideoModel(GatedDeltaProductVideoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config

        self.embeddings = VideoEmbeddings(config)
        self.encoder = GatedDeltaProductVideoEncoder(config)
...
class GatedDeltaProductVideoDecoder(nn.Module):
    def __init__(self, config, num_patches):
        ...
        self.decoder_blocks = nn.ModuleList(
            [
                GatedDeltaProductVideoBlock(decoder_config, layer_idx)
...
class GatedDeltaProductForVideoPreTraining(GatedDeltaProductVideoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
        self.backbone = GatedDeltaProductVideoModel(config)
        ...
        self.decoder = GatedDeltaProductVideoDecoder(
...
class GatedDeltaProductForVideoClassification(GatedDeltaProductVideoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

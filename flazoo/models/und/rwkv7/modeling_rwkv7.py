...
class RWKV7VisionChannelMixer(nn.Module):
...
class RWKV7VisionEncoder(nn.Module):
...
class RWKV7VisionModel(RWKV7VisionPreTrainedModel):
    def __init__(self, config, add_pooling_layer=True, use_mask_token=False):
        ...
        self.encoder = RWKV7VisionEncoder(config)
...
class RWKV7VideoChannelMixer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(config.hidden_size, config.channel_mixer_dim),
...
class RWKV7VideoModel(RWKV7VideoPreTrainedModel):
    def __init__(self, config):
        ...
        self.encoder = RWKV7VideoEncoder(config)

...
class HGRNVisionChannelMixer(nn.Module):
...
class HGRNVisionBlock(nn.Module):
    def __init__(self, config, layer_idx: int):
        ...
        self.channel_mixer = HGRNVisionChannelMixer(config)
...
class HGRNVideoChannelMixer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(config.hidden_size, config.channel_mixer_dim),
...
class HGRNVideoBlock(nn.Module):
    def __init__(self, config, layer_idx: int):
        super().__init__()
        ...
        self.channel_mixer = HGRNVideoChannelMixer(config)
        if config.attn is not None and layer_idx in config.attn['layers']:
            self.train_scan_type = 'uni-scan'
            self.test_scan_type = 'uni-scan'
        else:
...
class HGRNVideoEncoder(nn.Module):
    def __init__(self, config) -> None:
        super().__init__()
        self.config = config
        self.blocks = nn.ModuleList(
            [HGRNVideoBlock(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.gradient_checkpointing = False
...
class HGRNVideoPreTrainedModel(PreTrainedModel):
    config_class = HGRNVideoConfig
...
class HGRNVideoModel(HGRNVideoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config

        self.embeddings = VideoEmbeddings(config)
        self.encoder = HGRNVideoEncoder(config)
...
class HGRNVideoDecoder(nn.Module):
    def __init__(self, config, num_patches):
        ...
        self.decoder_blocks = nn.ModuleList(
            [HGRNVideoBlock(decoder_config, layer_idx) for layer_idx in range(decoder_config.num_hidden_layers)]
        )
...
class HGRNForVideoPreTraining(HGRNVideoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
        self.backbone = HGRNVideoModel(config)
        ...
        self.decoder = HGRNVideoDecoder(config, num_patches=self.backbone.embeddings.num_patches)
...
class HGRNForVideoClassification(HGRNVideoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

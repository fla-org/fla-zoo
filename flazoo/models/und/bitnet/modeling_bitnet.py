...
class BitNetVisionBlock(nn.Module):
    def __init__(self, config, layer_idx: int):
        ...
        self.channel_mixer = BitNetVisionChannelMixer(config)
...
class BitNetVideoBlock(nn.Module):
    def __init__(self, config, layer_idx: int):
        super().__init__()
        ...
        self.channel_mixer = BitNetVideoChannelMixer(config)
        if config.attn is not None and layer_idx in config.attn['layers']:
            self.train_scan_type = 'uni-scan'
            self.test_scan_type = 'uni-scan'
        else:
...
class BitNetVideoEncoder(nn.Module):
    def __init__(self, config) -> None:
        super().__init__()
        self.config = config
        self.blocks = nn.ModuleList(
            [BitNetVideoBlock(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.gradient_checkpointing = False
...
class BitNetVideoPreTrainedModel(PreTrainedModel):
    config_class = BitNetVideoConfig
...
class BitNetVideoModel(BitNetVideoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config

        self.embeddings = VideoEmbeddings(config)
        self.encoder = BitNetVideoEncoder(config)
...
class BitNetVideoDecoder(nn.Module):
    def __init__(self, config, num_patches):
        ...
        self.decoder_blocks = nn.ModuleList(
            [BitNetVideoBlock(decoder_config, layer_idx) for layer_idx in range(decoder_config.num_hidden_layers)]
        )
...
class BitNetForVideoPreTraining(BitNetVideoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
        self.backbone = BitNetVideoModel(config)
        ...
        self.decoder = BitNetVideoDecoder(config, num_patches=self.backbone.embeddings.num_patches)
...
class BitNetForVideoClassification(BitNetVideoPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

...
from flazoo.models.attentions import VisionXAttention
...
class XAttentionVisionMLP(nn.Module):
...
class XAttentionVisionBlock(nn.Module):
    def __init__(self, config, layer_idx: int):
        ...
        self.channel_mixer = XAttentionVisionMLP(config)
...
class XAttentionVisionPreTrainedModel(PreTrainedModel):
    config_class = XAttentionVisionConfig
...
class XAttentionVisionEncoder(nn.Module):
    def __init__(self, config) -> None:
        super().__init__()
        self.config = config
        self.blocks = nn.ModuleList([
            XAttentionVisionBlock(config, layer_idx) 
...
class XAttentionVisionModel(XAttentionVisionPreTrainedModel):
    def __init__(self, config, add_pooling_layer=True, use_mask_token=False):
        super().__init__(config)
        self.config = config
        self.embeddings = ImageEmbeddings(config, use_mask_token=use_mask_token)
        self.encoder = XAttentionVisionEncoder(config)
...
class XAttentionForImageClassification(XAttentionVisionPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_classes
        self.backbone = XAttentionVisionModel(config, add_pooling_layer=True) # Here we should use mean pooling
...
class XAttentionForMaskedImageModeling(XAttentionVisionPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.backbone = XAttentionVisionModel(config, add_pooling_layer=False, use_mask_token=True) 

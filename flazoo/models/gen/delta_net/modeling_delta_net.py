...
class DeltaNetGen2DBlock(nn.Module):
    ...
    def forward(self, x, c):
        shift_attn, scale_attn, gate_attn, shift_channel_mixer, scale_channel_mixer, gate_channel_mixer = (
            self.adaLN_modulation(c).chunk(6, dim=-1)
        )
        
        x = x + gate_attn.unsqueeze(1) * self.attn(modulate(self.norm1(x), shift_attn, scale_attn))
        x = x + gate_channel_mixer.unsqueeze(1) * self.channel_mixer(modulate(self.norm2(x), shift_channel_mixer, scale_channel_mixer))

...
from fla.ops import fused_recurrent_delta_rule, chunk_delta_rule
from flazoo.ops import generate_sta_mask_3d, sta_3d_with_text_func
import warnings
...
class DeltaNetCrossAttentionHF(Attention):
    ...
    def __init__(self, fla_config: Dict = None, layer_idx: int = 0, *args, **kwargs):
        ...
        if self.use_gate:
            self.g_proj = nn.Linear(
                self.query_dim, self.cross_attention_dim, bias=False
            )
    ...
    def op_forward_func(
        ...
        **kwargs: Unpack[Dict],
        ...
        v = rearrange(
            v, "... (h d) -> ... h d", h=self.heads
        )  # TODO: support different kv heads in the future
        if self.qk_activation != "silu":
...
class SlidingTileCrossAttentionHF3D(Attention):
    ...
    def __init__(self, fla_config: Dict = None, layer_idx: int = 0, *args, **kwargs):
        ...
        assert fla_config["attn"] is not None, (
            "fla_config.attn must be provided for STA3DCrossAttentionHF."
        )
        attn_config = fla_config["attn"]

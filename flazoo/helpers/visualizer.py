...
class ModelVisualizer:
    ...
    def register_attention_hooks(self, layer_indices: List[int]):
        ...
        for layer_idx in layer_indices:
            if not hasattr(self.model, 'encoder') or not hasattr(self.model.encoder, 'blocks'):
                raise ValueError("Model structure not compatible with this visualizer")

            if layer_idx >= len(self.model.encoder.blocks):
                raise ValueError(f"Layer index {layer_idx} out of range (model has {len(self.model.encoder.blocks)} layers)")
            ...
            if isinstance(output, tuple) and len(output) >= 2 and output[1] is not None:
                ...
                if self.target_batch_idx is not None:
                    ...
                    if output[1].dim() == 4:  # [batch, heads, seq_len, seq_len]
                        self.attention_outputs[layer_idx] = output[1][self.target_batch_idx].detach().cpu()
                    else:
    ...
    def visualize_attention(self,
        ...
        for layer_idx, attention in self.attention_outputs.items():
            ...
            if attention.dim() == 3:  # [heads, seq_len, seq_len]
                ...
                batch_idx = 0 if self.target_batch_idx is None else self.target_batch_idx
    ...
    def _visualize_single_attention(self,
        ...
        cmap, figsize, bg_color, text_color, edge_color, title_size, subtitle_size = self._get_style_params(style)
        ...
        fig, axes = plt.subplots(grid_size, grid_size, figsize=figsize, facecolor=bg_color)
        ...
        for head_idx in range(num_heads):
            if head_idx < len(axes):
                ...
                # Style the subplot
                ax.set_title(f"Head {head_idx}", color=text_color, fontsize=subtitle_size)
                ax.set_xticks([])
                ax.set_yticks([])
        ...
        # Add a main title
        plt.suptitle(f"Layer {layer_idx} Attention", color=text_color, fontsize=title_size, y=0.98)

        # Save the figure
        filename = f"{filename_prefix}_layer{layer_idx}.png"
        plt.savefig(os.path.join(output_dir, filename), dpi=dpi, bbox_inches='tight', facecolor=bg_color)
        ...
        plt.title(f"Layer {layer_idx} - Average Attention", color=text_color, fontsize=title_size)
        ...
        if style == VisualizationStyle.ELEGANT or style == 'elegant':
            ...
            for i, j in zip(*hotspots):
                plt.axhline(y=i, color=edge_color, linestyle='-', alpha=0.3, linewidth=0.5)
                plt.axvline(x=j, color=edge_color, linestyle='-', alpha=0.3, linewidth=0.5)

        plt.tight_layout()

        avg_filename = f"{filename_prefix}_layer{layer_idx}_avg.png"
        plt.savefig(os.path.join(output_dir, avg_filename), dpi=dpi, bbox_inches='tight', facecolor=bg_color)
    ...
    def _get_style_params(self, style):
        ...
        if style == VisualizationStyle.STANDARD:
            ...
            return ('pastel', 'white', '#ff9aa2', '#c7ceea'), (15, 15), 'white', '#2e4057', '#2e4057', 18, 12

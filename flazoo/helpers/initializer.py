...
def initialize_from_pretrained(
    ...
    if backbone_a is None or backbone_b is None:
        raise ValueError(
            f"Could not find backbone in models. Looked for attributes: {backbone_attr}"
        )
    ...
    if num_layers_a > num_layers_b:
        raise ValueError(
            f"Model A has more layers ({num_layers_a}) than Model B ({num_layers_b})"
        )
...
def _find_backbone(
    model: nn.Module, backbone_attr: List[str]
    ...
    def search_backbone(
        module: nn.Module, depth: int = 0, max_depth: int = 5
    ) -> Optional[nn.ModuleList]:
        if depth > max_depth:
            return None
...
def _initialize_parameter_in_a_layer(
    layer_a: nn.Module, layer_b: nn.Module, param_a_name: str, param_b_name: str
) -> None:
    """
    ...
    if isinstance(param_a, nn.Module) and isinstance(param_b, nn.Module):
        ...
        if hasattr(param_a, "weight") and hasattr(param_b, "weight"):
            ...
            if param_a.weight.shape != param_b.weight.shape:
                raise ValueError(
                    f"Weight shape mismatch: {param_a.weight.shape} vs {param_b.weight.shape}"
                )
            with torch.no_grad():
            ...
            if (
                hasattr(param_a, "bias")
                and hasattr(param_b, "bias")
                and param_a.bias is not None
                and param_b.bias is not None
            ):
                if param_a.bias.shape != param_b.bias.shape:
                    raise ValueError(
                        f"Bias shape mismatch: {param_a.bias.shape} vs {param_b.bias.shape}"
                    )
                with torch.no_grad():
        ...
        if param_a.shape != param_b.shape:
            raise ValueError(
                f"Parameter shape mismatch: {param_a.shape} vs {param_b.shape}"
            )
        with torch.no_grad():
        ...
        if hasattr(param_a, "weight"):
            if param_a.weight.shape != param_b.shape:
                raise ValueError(
                    f"Shape mismatch: {param_a.weight.shape} vs {param_b.shape}"
                )
            with torch.no_grad():
        ...
        if hasattr(param_b, "weight"):
            if param_a.shape != param_b.weight.shape:
                raise ValueError(
                    f"Shape mismatch: {param_a.shape} vs {param_b.weight.shape}"
                )
            with torch.no_grad():
...
def initialize_mlp_params(
    ...
    mapping = {f"{fc1_name_a}": f"{fc1_name_b}", f"{fc2_name_a}": f"{fc2_name_b}"}

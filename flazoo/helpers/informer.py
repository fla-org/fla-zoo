...
def print_parameter_count(model: nn.Module) -> None:
    ...
    if counts['total'] > 0:
        print(f"Trainable Percentage: {counts['trainable']/counts['total']*100:.2f}%")
    print("=" * 50)
...
def print_parameter_shapes(model: nn.Module, filter_str: Optional[str] = None) -> None:
    ...
    for name, param in sorted(model.named_parameters(), key=lambda x: _natural_sort_key(x[0])):
...
def log_model_parameters(model: nn.Module, log_path: str = "model_parameters.log") -> None:
    """
    ...
    with open(log_path, 'w') as f:
        ...
        if counts['total'] > 0:
            f.write(f"Trainable Percentage: {counts['trainable']/counts['total']*100:.2f}%\n")
        f.write("\n" + "=" * 100 + "\n\n")
...
def _write_module_hierarchy(model: nn.Module, hierarchy: Dict[str, List[str]],
                           file, parent_name: str = '', level: int = 0) -> None:
    """
    ...
    for child_name in sorted(children, key=_natural_sort_key):
        ...
        if direct_params:
            ...
            module_trainable_params = sum(p.numel() for p in direct_params.values() if p.requires_grad)
            ...
            for param_name, param in sorted(direct_params.items(), key=lambda x: _natural_sort_key(x[0])):
                shape_str = str(tuple(param.shape))
                size = param.numel()
                trainable = "✓" if param.requires_grad else "✗"
                file.write(f"{indent}  - {param_name}: {shape_str}, {size:,} elements, trainable: {trainable}\n")

        # Process children recursively
        _write_module_hierarchy(model, hierarchy, file, child_name, level + 1)


def log_model_parameters_flat(model: nn.Module, log_path: str = "model_parameters_flat.log") -> None:
    """
    ...
    with open(log_path, 'w') as f:
        ...
        # Write table header
        f.write(f"{'Parameter Name':<60} {'Shape':<20} {'Size':>12} {'Trainable':<10} {'Type':<10}\n")
        f.write("-" * 100 + "\n")
        ...
        for name, param in sorted(model.named_parameters(), key=lambda x: _natural_sort_key(x[0])):
            ...
            f.write(f"{name:<60} {shape_str:<20} {size:>12,} {trainable:<10} {dtype:<10}\n")
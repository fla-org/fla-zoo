...
def train_one_epoch(
    ...
    mixup_fn=None, model_ema=None, training_args=None, accelerator=None, lr_scheduler=None, teacher=None
):
    """Train for one epoch"""
    model.train()
    ...
    for batch_idx, (inputs, targets) in enumerate(loader):
        ...
        if teacher != None:
            with torch.no_grad():
                targets = teacher(pixel_values=inputs).last_hidden_state
...
def validate(model, loader, loss_fn, device, training_args, accelerator=None, teacher=None):
    ...
    if training_args.init_from_pretrained:
        if training_args.init_model == "dino":
            ...
            if training_args.training_mode == "label":
                ...
                logging.info("Retaining SigLIP for distillation")
